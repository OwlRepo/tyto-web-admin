...
const CreateAccountModal = ({isOpen,onClose, scheduleIDS}) => {
  ...
  const processCreateAccount = async () => {
    const createAccountResult = await createTeacherAccount({email:email,fullname:fullname,password:password,schedule_id:scheduleID})
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <VStack paddingY={'10'} spacing={'5'}>
              <Box width={'100%'}>
                <Text>Email</Text>
                <Input variant={'filled'} placeholder={'sample@email.com'} onChange={event=>setEmail(event.target.value)}/>
              </Box>
              <Box width={'100%'}>
                <Text>Full Name</Text>
                <Input variant={'filled'} placeholder={'Juan Dela Cruz'} onChange={event=>setFullname(event.target.value)}/>
              </Box>
              <Box width={'100%'}>
                ...
                <Menu >
                  ...
                  <MenuList >
                    {scheduleIDS.map((data,index)=>{
                      return (
                        <MenuItem onClick={()=>setScheduleID(data.id)}>{data.id}</MenuItem>
                      )
                    })}
                  </MenuList>
                </Menu>
              ...
              <Box width={'100%'}>
                <Text>Password</Text>
                <Input type={'password'} variant={'filled'} placeholder={'****'} onChange={event=>setPassword(event.target.value)}/>
              </Box>
            </VStack>
          </ModalBody>
          <ModalFooter>
